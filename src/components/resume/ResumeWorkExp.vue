...
<template>
    <section>
        ...
        >
            <span style="color: #444440">Experience</span>
        ...
        <div
            v-if="isWorkExpOpen"
        >
            <ul class="grid grid-cols-1 gap-4 text-gray-600 mb-4">
                <li
                ...
                >
                    ...
                    <template v-if="exp.promotionFrom">
                        <div class="mb-2">
                            <h4 class="text-lg md:text-xl font-semibold text-gray-500">{{ exp.company }}</h4>
                        </div>
                        <div class="grid grid-cols-[16px_1fr] gap-x-4">
                            ...
                            <div class="relative">
                                ...
                                <span class="absolute left-1/2 -translate-x-1/2 top-1.5 h-3 w-3 rounded-full bg-gray-900"></span>
                            </div>
                            <!-- Current role content -->
                            <div>
                                <div class="font-medium">{{ exp.title }}</div>
                                <div class="text-xs text-[rgb(130,130,6)]">{{ exp.dateRange }}</div>
                                <ul class="text-sm text-gray-500 list-none ml-4 mt-2">
                            ...
                            <div class="relative">
                                <div class="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-gray-300"></div>
                                <span class="absolute left-1/2 -translate-x-1/2 top-1.5 h-3 w-3 rounded-full bg-gray-400"></span>
                            </div>
                    ...
                    <template v-else>
                        <h4 class="text-lg md:text-xl font-semibold mb-1 text-gray-500">{{ exp.company }}</h4>
                        <div class="font-medium">{{ exp.title }}</div>
                        <div class="text-xs text-[rgb(130,130,6)]">{{ exp.dateRange }}</div>
                        <ul class="text-sm text-gray-500 list-none ml-4 mt-2">
                            <li v-for="(responsibility, index) in exp.responsibilities" :key="index" class="flex items-start">
